...
function updateSkillTreeUI() {
    ...
    if (milestoneEl) {
        ...
        if (nextMilestone) {
            const btcProgressPercent = Math.min(((btcLifetime / nextMilestone.btc) * 100), 100).toFixed(1);
            const ethProgressPercent = Math.min(((ethLifetime / nextMilestone.eth) * 100), 100).toFixed(1);
            const dogeProgressPercent = Math.min(((dogeLifetime / nextMilestone.doge) * 100), 100).toFixed(1);
            ...
            milestoneEl.innerHTML = `
                ...
                <div style="font-size: 0.75rem; color: #aaa;">
                    ₿ ${btcLifetime.toFixed(8)} / ${nextMilestone.btc} (${btcProgressPercent}%)<br>
                    Ξ ${ethLifetime.toFixed(8)} / ${nextMilestone.eth} (${ethProgressPercent}%)<br>
                    Ð ${dogeLifetime.toFixed(8)} / ${nextMilestone.doge} (${dogeProgressPercent}%)
                </div>
            `;
        } else {
            milestoneEl.innerHTML = `
